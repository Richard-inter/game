...
import (
	...
	"github.com/Richard-inter/game/internal/service/rpc/gachaMachine"
...
func main() {
	...
	log.Infow("Starting GachaMachine Runtime Service",
		"version", Version,
		"buildTime", BuildTime,
		"goVersion", GoVersion,
	)
	...
	if configFile == "" {
		configFile = "config/rpc-gachamachine-runtime-service.yaml" // fallback
	}
	...
	// Initialize and register gacha machine runtime service
	...
	// Initialize the actual gachaMachine service
	gachaMachineSvc := gachaMachine.NewGachaMachineGRPCService(gachaMachineRepo, redisClient, "")

	runtimeService := c.NewGachaMachineWebsocketService(gachaMachineSvc)
	...
	log.Infow("Shutting down GachaMachine Runtime service...")
	...
	select {
	case <-done:
		log.Infow("GachaMachine Runtime service stopped gracefully")
	case <-ctx.Done():
		log.Infow("GachaMachine Runtime service shutdown timeout")
		s.Stop()
	}
}
